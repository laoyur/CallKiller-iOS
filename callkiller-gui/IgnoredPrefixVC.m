...
#import "IgnoredPrefixVC.h"
#import "Preference.h"
#import "AlertUtil.h"

@interface IgnoredPrefixVC ()
@property (weak, nonatomic) IBOutlet UIBarButtonItem *trashButton;
@property (weak, nonatomic) IBOutlet UITableView *tableview;
@property (weak, nonatomic) UIAlertAction *alertAction;
@end

@implementation IgnoredPrefixVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    NSDictionary *pref = [[Preference sharedInstance] pref];
    NSArray *list = pref[kKeyIgnoredPrefixes];
    self.trashButton.enabled = list.count > 0;
}
...
- (IBAction)onTrash:(id)sender {
    [AlertUtil showQuery:NSLocalizedString(@"del-all-prefixes-prompt", nil) message:nil isDanger:YES onConfirm:^{
        NSMutableDictionary *pref = [[Preference sharedInstance] pref];
        NSMutableArray *list = pref[kKeyIgnoredPrefixes];
...
- (IBAction)onAdd:(id)sender {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"add-prefix-title", nil) message:NSLocalizedString(@"add-prefix-content", nil) preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:NSLocalizedString(@"ok", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (number.length) {
            ...
            pattern = [NSString stringWithFormat:@"^%@\\d+", pattern];
            
            NSMutableDictionary *pref = [[Preference sharedInstance] pref];
            NSMutableArray *prefixes = pref[kKeyIgnoredPrefixes];
            if (!prefixes) {
                prefixes = [NSMutableArray new];
                pref[kKeyIgnoredPrefixes] = prefixes;
            }
            for (NSArray *item in prefixes) {
                if ([item[0] isEqualToString:number]) {
                    [AlertUtil showInfo:NSLocalizedString(@"duplicated-entry", nil) message:nil];
                    return;
                }
            }
            [prefixes addObject:@[number, pattern, [alert.textFields[1].text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]]];
            [[Preference sharedInstance] save];
            [self.tableview reloadData];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [actions addObject:[UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal title:NSLocalizedString(@"comment", nil) handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        NSMutableDictionary *pref = [[Preference sharedInstance] pref];
        NSMutableArray *list = pref[kKeyIgnoredPrefixes];
        NSMutableArray *item = [list[indexPath.row] mutableCopy];
